...
if __name__ == "__main__":
    ...
    url_propose  = f"{api_base}/instance/{instance_uuid}/propose"
...
# Debug helpers
    ...
    status_url = data.get("status_url") or resp.headers.get("Location")


    # ... after extracting task_id/status_url from the POST response ...
    task_id = data.get("task_id")
    status_url = data.get("status_url") or resp.headers.get("Location")
    if not status_url and task_id:
        status_url = f"{api_base_tasks.rstrip('/')}/{task_id}/"

    #  ensure trailing slash so Django doesn't 301
    if not status_url.endswith('/'):
        status_url += '/'
    ...
    # now poll
    exp_list = wait_for_exp_list(status_url, timeout=300, interval=2.0)
...
#    # Fallback: construct from task_id if we know the tasks base
#    if not status_url and task_id:
#        status_url = f"{api_base_tasks.rstrip('/')}/{task_id}/"
#
#    if not status_url:
#        raise RuntimeError("Could not determine status_url from response. "
#                           "Ensure /backend/<instance_uuid>/all_proposed returns 202 with task_id/status_url or Location header.")
#
#
#    # ensure trailing slash
#    if not status_url.endswith('/'):
#        status_url += '/'
#        resp = requests.get(status_url, headers={"Accept": "application/json"},
...
#    print("Status URL:", status_url)
#
#    # --- Poll until done and get exp_list ---
#    exp_list = wait_for_exp_list(status_url, token=env.get("DM_AUTH_TOKEN"), timeout=300, interval=2.0)
