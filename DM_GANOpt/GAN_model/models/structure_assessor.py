...
def differentiable_resintrap_mask(voxel_grid, iterations=20, steepness=10.0, threshold=0.5):
    """
    Returns:
        - resintrap_mask: soft mask (B, 1, D, H, W)
        - num_resin_traps: int, number of *distinct* resin trap regions
    """
    B, C, D, H, W = voxel_grid.shape
    device = voxel_grid.device

    void_grid = 1.0 - voxel_grid.to(device)

    # Boundary seeding
    seed = torch.zeros_like(void_grid)
    seed[:, :, 0, :, :] = seed[:, :, -1, :, :] = 1
    seed[:, :, :, 0, :] = seed[:, :, :, -1, :] = 1
    seed[:, :, :, :, 0] = seed[:, :, :, :, -1] = 1
    ...
    kernel[0, 0, 1, 1, [0, 2]] = 1
    kernel[0, 0, 1, [0, 2], 1] = 1
    kernel[0, 0, [0, 2], 1, 1] = 1

    mask = seed.clone()
    for _ in range(iterations):
        conv = F.conv3d(mask, kernel, padding=1)
        activated = torch.sigmoid(steepness * (conv - threshold))
        mask = torch.where(void_grid > 0, torch.max(mask, activated), mask)

    resintrap_mask = void_grid * (1.0 - mask)

    # Non-differentiable region counting
    with torch.no_grad():
        binary_mask = (resintrap_mask > 0.5).float()
        binary_np = binary_mask[0, 0].cpu().numpy().astype(np.uint8)
        ...
        labeled, num_regions = label(binary_np, structure=structure)


    binary_mask = (resintrap_mask > 0.5).float()
    num_trap_voxels = int(binary_mask.sum().item())

    return resintrap_mask, num_trap_voxels #num_regions
...
class Assessor(nn.Module):
    ...
    def resin_trap_score(self, voxel_grid, resintrap_weight=100.0):
        resin_mask, num_resin_traps = differentiable_resintrap_mask(voxel_grid)
        score = resintrap_weight * resin_mask.sum() / voxel_grid.numel()
        return resin_mask, score
